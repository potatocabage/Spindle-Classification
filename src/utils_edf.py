...
def read_raw(raw_path, resample=2000, drop_duplicates=True):
    ...
    sample_rate_ratio = resample / raw.info['sfreq']
    ...
    return data, channels, sample_rate_ratio

def concate_edf(fns, resample):
    data, channel = [], []
    for fn in fns:
        # assumes that the resample ratio is the same for all files
        d, c, r = read_raw(fn, resample = resample)
        data.append(d), channel.append(c)
    data = np.concatenate(data)
    channel = np.concatenate(channel)
    return data, channel, r
